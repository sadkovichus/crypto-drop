...
import { generateMetaTags } from '../model';
import { useDynamicMeta } from '@shared/hooks';

export const RootLayout = () => {
  const location = useLocation();
  const pathname = location.pathname.endsWith('/') ? location.pathname.slice(0, -1) : location.pathname;
  const { dynamicMeta } = useDynamicMeta(); // Деструктурируем только dynamicMeta
  const metaTags = generateMetaTags(pathname || '/', dynamicMeta); // Передаем только dynamicMeta
